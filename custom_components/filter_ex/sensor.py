...
PLATFORM_SCHEMA = SENSOR_PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional("max_sub_interval"): cv.time_period,
...
async def async_setup_platform(
    ...
    async_add_entities([SensorFilter(name, unique_id, entity_id, filters, config)])
...
class SensorFilter(SensorEntity):
    """Representation of a Filter Sensor with optional forced update (max_sub_interval)."""
    ...
    def __init__(
        ...
        self._max_sub_interval: timedelta | None = config.get("max_sub_interval")
        self._last_update: datetime | None = None
        self._unsub_check: callable | None = None

        self._attr_available = False  # sensor is inactive until first valid update
    ...
    def _update_filter_sensor_state(
        ...
        *,
        ...
        """Process device state changes or forced updates."""
        ...
        if new_state.state in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            _LOGGER.debug(
                "%s: source %s is %s", self.name, self._entity, new_state.state
            )
        ...
        last_updated = dt_util.utcnow() if forced else (new_state.last_updated or dt_util.utcnow())
        self._last_update = last_updated
        self._attr_available = True

        temp_state = _State(last_updated, new_state.state)
        ...
        # Copy attributes from the source
        ...
        if update_ha:
            if forced:
                self._attr_extra_state_attributes["last_forced_update"] = (
    ...
    @callback
    def _check_stale_source(self, now: datetime) -> None:
        """Check if the source entity is stale and trigger forced update."""
        if not self._max_sub_interval or not self._last_update:
            return

        elapsed = dt_util.utcnow() - self._last_update
        if elapsed >= self._max_sub_interval:
            _LOGGER.debug(
                "%s: source entity %s stale for %s — pushing same state again",
                self.name,
                self._entity,
                elapsed,
            )
            ...
            if state and state.state not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
                self._update_filter_sensor_state(state, update_ha=True, forced=True)
    ...
    async def async_added_to_hass(self) -> None:
        """Register callbacks and restore filter history."""
        ...
        if "recorder" in self.hass.config.components:
            ...
            # Determine the largest window size among all filters
            for filt in self._filters:
                if (
            ...
            # Load event-based window history
            ...
            # Load time-based window history
            ...
            # Sort the history list chronologically
            history_list = sorted(history_list, key=lambda s: s.last_updated)
            _LOGGER.debug(
                "Loading history for %s: %s",
                self.name,
                [(s.state, s.last_updated) for s in history_list],
            )

            # Replay history through filters
            for state in history_list:
                if state.state not in [STATE_UNKNOWN, STATE_UNAVAILABLE, None]:
                    self._update_filter_sensor_state(state, update_ha=False)

        # Start periodic stale check
        if self._max_sub_interval:
            self._unsub_check = async_track_time_interval(
                self.hass, self._check_stale_source, self._max_sub_interval
            )
            self.async_on_remove(self._unsub_check)

        # Delay subscription until first valid state
        @callback
        def _async_hass_started(hass: HomeAssistant) -> None:
            """Subscribe to source entity updates after HA is fully started."""
            _LOGGER.debug(
                "%s: subscribing to state changes from %s", self.name, self._entity
            )
